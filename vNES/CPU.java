...
public final class CPU implements Runnable{
	...
	public synchronized void beginExecution(){
		...
		myThread = new Thread(this, "Emulation thread");
    initRun();
		myThread.start();
	...
	public synchronized void endExecution(){
		...
		if(myThread!=null && myThread.isAlive()){
			try{
				stopRunning = true;
        System.out.println("joining cpu thread");
				myThread.join();
        System.out.println("joined cpu thread");
	...
	public void emulate(){
		...
		//stopRunning = false;
                System.out.println("emulating at SP "+Integer.toHexString(REG_SP) );
		...
		while(true){
            ...
            for (int n = opaddr; n < (opaddr + 10); n++)
            {
              System.out.println(Integer.toHexString(n)+":"+Integer.toHexString(mem[n]));
            }
					}
					break;
