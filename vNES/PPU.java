...
public class PPU {
  ...
  public boolean firstWrite = true; 		// VRAM/Scroll Hi/Lo latch
  ...
  public void startVBlank() {
    ...
    if (lastRenderedScanline < 239) {
      renderFramePartially(getBuffer(), lastRenderedScanline + 1, 240 - lastRenderedScanline);
    }

    endFrame();

    // Notify image buffer:
    if (!Globals.screenshotMode) {
    ...
    startFrame();
    if (!Globals.screenshotMode) {
      synchronized (Gametape.evaluator) {
        if (Gametape.running) {
          Gametape.evaluator.notify();
        }
      }
    }
  ...
  private void renderFramePartially(int[] buffer, int startScan, int scanCount) {
    ...
    if (!Globals.screenshotMode && screen.scalingEnabled() && !screen.useHWScaling() && !requestRenderAll) {
  ...
  public void renderPattern() {
    ...
    for (int j = 0; j < 2; j++) {
      for (int y = 0; y < 16; y++) {
        for (int x = 0; x < 16; x++) {
          ptTile[tIndex].renderSimple(j * 128 + x * 8, y * 8, buffer, 0, (j == 1 ? imgPalette : sprPalette));
          tIndex++;
        }
      }
    }
      ...
      for (int ntx = 0; ntx < ntx_max; ntx++) {
        ...
        for (int ty = 0; ty < 30; ty++) {
          for (int tx = 0; tx < 32; tx++) {
            //ptTile[baseTile+nameTable[nt].getTileIndex(tx,ty)].render(0,0,8,8,x+tx*8,y+ty*8,buffer,nameTable[nt].getAttrib(tx,ty),imgPalette,false,false,0,dummyPixPriTable);
            //ptTile[baseTile+nameTable[nt].getTileIndex(tx,ty)].renderSmall(x+tx*4,y+ty*4,buffer,nameTable[nt].getAttrib(tx,ty),imgPalette);
            ptTile[baseTile + nameTable[nt].getTileIndex(tx, ty)].renderNames(x + tx * 8, y + ty * 8, buffer, nameTable[nt].getAttrib(tx, ty), imgPalette);
          }
        }
  ...
  public void updatePalettes() {
    ...
    if (Globals.PPUDebug && nes.getGui().getImgPalView() != null) {
      renderPalettes();
    }
  ...
  public int[] renderFullFrame() {
    renderSpritesPartially(0, 240, true);
    for (int y = 1; y < 240; y++) {
      renderBgScanline(getBuffer(), y);
    }
    renderSpritesPartially(0, 240, false);
    //renderFramePartially(getBuffer(), 0, 240);
    return getBuffer();
  }
  ...
  public int[] getBuffer() {
    if (Globals.screenshotMode) {
      if (screenshotRenderBuffer == null) {
        screenshotRenderBuffer = new int[256 * 240];
      }
      return screenshotRenderBuffer;
    } else {
      return nes.getGui().getScreenView().getBuffer();
    }
  }
