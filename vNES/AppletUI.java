...
public class AppletUI implements UI{
	...
	ScreenView vScreen;
        BufferView patternView;
        BufferView spritePaletteView;
        BufferView nameTableView;
        BufferView imagePaletteView;
	HiResTimer timer;
	...
	public void init(boolean showGui){
            if (Globals.PPUDebug) {
                patternView = new BufferView(nes, 256, 128);
                nameTableView = new BufferView(nes, 512, 480);
                spritePaletteView = new BufferView(nes, 256, 16);
                imagePaletteView = new BufferView(nes, 256, 16);

		patternView.setBgColor(applet.bgColor.getRGB());
		patternView.init();

		nameTableView.setBgColor(applet.bgColor.getRGB());
		nameTableView.init();

		spritePaletteView.setBgColor(applet.bgColor.getRGB());
		spritePaletteView.init();

		imagePaletteView.setBgColor(applet.bgColor.getRGB());
		imagePaletteView.init();
            }
